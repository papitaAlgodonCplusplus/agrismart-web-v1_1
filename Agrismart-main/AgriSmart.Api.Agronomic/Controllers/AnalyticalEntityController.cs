...
namespace AgriSmart.API.Agronomic.Controllers
{
    ...
    public class AnalyticalEntityController : ControllerBase
    {
        ...
        /// <summary>
        /// Delete an analytical entity (soft delete)
        /// </summary>
        /// <param name="Id">Analytical Entity ID to delete</param>
        /// <returns></returns>
        [HttpDelete("{Id:int}")]
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status401Unauthorized)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        public async Task<ActionResult<Response<DeleteAnalyticalEntityResponse>>> Delete([FromRoute] int Id)
        {
            var command = new DeleteAnalyticalEntityCommand { Id = Id };
            var response = await _mediator.Send(command);
