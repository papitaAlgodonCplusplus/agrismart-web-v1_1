...
namespace AgriSmart.API.Agronomic.Controllers
{
    ...
    public class GrowingMediumController : ControllerBase
    {
        ...
        /// <summary>
        /// Delete a growing medium (soft delete)
        /// </summary>
        /// <param name="Id">Growing medium ID to delete</param>
        /// <returns></returns>
        [HttpDelete("{Id:int}")]
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status401Unauthorized)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        public async Task<ActionResult<Response<DeleteGrowingMediumResponse>>> Delete([FromRoute] int Id)
        {
            var command = new DeleteGrowingMediumCommand { Id = Id };
            var response = await _mediator.Send(command);
