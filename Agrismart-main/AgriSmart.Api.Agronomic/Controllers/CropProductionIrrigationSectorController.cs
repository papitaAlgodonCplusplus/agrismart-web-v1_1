...
namespace AgriSmart.API.Agronomic.Controllers
{
    ...
    public class CropProductionIrrigationSectorController : ControllerBase
    {
        ...
        /// <summary>
        /// Delete a crop production irrigation sector (soft delete)
        /// </summary>
        /// <param name="Id">Crop production irrigation sector ID to delete</param>
        /// <returns></returns>
        [HttpDelete("{Id:int}")]
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status401Unauthorized)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        public async Task<ActionResult<Response<DeleteCropProductionIrrigationSectorResponse>>> Delete([FromRoute] int Id)
        {
            var command = new DeleteCropProductionIrrigationSectorCommand { Id = Id };
            var response = await _mediator.Send(command);
