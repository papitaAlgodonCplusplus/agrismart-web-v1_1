...
namespace AgriSmart.API.Agronomic.Controllers
{
    ...
    public class FertilizerController : ControllerBase
    {
        ...
        /// <summary>
        /// Delete a fertilizer (soft delete)
        /// </summary>
        /// <param name="Id">Fertilizer ID to delete</param>
        /// <returns></returns>
        [HttpDelete("{Id:int}")]
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status401Unauthorized)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        public async Task<ActionResult<Response<DeleteFertilizerResponse>>> Delete([FromRoute] int Id)
        {
            var command = new DeleteFertilizerCommand { Id = Id };
            var response = await _mediator.Send(command);
