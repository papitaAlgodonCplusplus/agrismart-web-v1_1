...
class LinearProgrammingOptimizer:
    ...
    def optimize_fertilizer_solution(self,
        ...
        if apply_safety_caps:
            ...
            caps_result = apply_nutrient_caps_to_targets(
                target_concentrations, strict_caps)
            safe_targets = caps_result['capped_concentrations']
            print(
                f"Safety caps applied: {caps_result['total_adjustments']} adjustments")

        # Try PuLP first, then SciPy
        if PULP_AVAILABLE:
            result = self._optimize_with_pulp(
                fertilizers, safe_targets, water_analysis, volume_liters)
        elif SCIPY_AVAILABLE:
            result = self._optimize_with_scipy(
                fertilizers, safe_targets, water_analysis, volume_liters)
        else:
            raise ImportError(
                "Neither PuLP nor SciPy available for linear programming")
        ...
        print(
            f"Average deviation: {np.mean(list(result.deviations_percent.values())):.2f}%")
    ...
    def _optimize_with_pulp(self,
        ...
        for fert in fertilizers:
            ...
            dosage_vars[fert.name] = lp.LpVariable(
                var_name, lowBound=0, upBound=self.max_individual_dosage)
        ...
        for nutrient in fertilizer_targets.keys():
            ...
            deviation_vars_pos[nutrient] = lp.LpVariable(
                f"dev_pos_{var_name}", lowBound=0)
            deviation_vars_neg[nutrient] = lp.LpVariable(
                f"dev_neg_{var_name}", lowBound=0)
        ...
        for nutrient in fertilizer_targets.keys():
            ...
            if target_value > 0:
                ...
                objective += weight * \
                    (deviation_vars_pos[nutrient] +
                     deviation_vars_neg[nutrient])
        ...
        for nutrient in fertilizer_targets.keys():
            ...
            for fert in fertilizers:
                ...
                if total_content > 0:
                    # Convert dosage to contribution: g/L * (content% * purity% / 10000) * 1000 mg/g
                    # Simplified: g/L * content% * purity% * 0.1
                    contribution_factor = total_content * fert.chemistry.purity * 0.1
        ...
        try:
            ...
            if prob.status == lp.LpStatusOptimal:
               ...
               # Extract dosages
                ...
                # FIXED: Calculate deviations against ORIGINAL TARGETS (not fertilizer-only targets)
                # The targets passed in are already the desired FINAL concentrations
                deviations_percent = {}
                for nutrient in targets.keys():
                    # This is the desired final concentration
                    target_final = targets[nutrient]
                    achieved_final = achieved_concentrations.get(
                        nutrient, 0.0)  # This includes water + fertilizers

                    if target_final > 0:
                        deviation = (
                            (achieved_final - target_final) / target_final) * 100.0
                    else:
                        deviation = 0.0 if achieved_final == 0 else 100.0
                    deviations_percent[nutrient] = deviation

                    # Debug output
                    water_contrib = water.get(nutrient, 0.0)
                    print(
                        f"[LP] {nutrient}: Target={target_final:.2f}, Achieved={achieved_final:.2f} (Water={water_contrib:.2f}), Dev={deviation:.2f}%")

                # Calculate ionic balance
                ionic_balance_error = self._calculate_ionic_balance_error(
                ...
                active_fertilizers = len(
                    [d for d in cleaned_dosages.values() if d > 0])
    ...
    def _optimize_with_scipy(self,
        ...
        for nutrient, total_target in targets.items():
            ...
            if water_contribution > 0:
                print(
                    f"  {nutrient}: Total={total_target:.3f} - Water={water_contribution:.3f} = Fertilizer={fertilizer_target:.3f} mg/L")
            else:
                print(
                    f"  {nutrient}: Total={total_target:.3f} mg/L (no water contribution)")

        # Create matrices for linear programming
        # FIXED: Use fertilizer targets
        nutrient_list = list(fertilizer_targets.keys())
        ...
        for i, nutrient in enumerate(nutrient_list):
            ...
            if total_content > 0:
                ...
                # Formula: (content% * purity% / 10000) * 1000 mg/g = content% * purity% * 0.1
                contribution_factor = total_content * fertilizer.chemistry.purity * 0.1
        ...
        def objective(x):
            ...
            fertilizer_targets_array = np.array(
                [fertilizer_targets[nutrient] for nutrient in nutrient_list])

            # Calculate weighted deviations against fertilizer targets
            deviations = np.abs(achieved_fertilizer - fertilizer_targets_array) / \
                np.maximum(fertilizer_targets_array, 1e-6)

            # Primary objective: minimize deviations
            deviation_penalty = np.sum(
                deviations ** 2) * self.objective_weights['deviation_minimization']

            # Secondary objective: minimize total dosage
            dosage_penalty = np.sum(
                dosages) * self.objective_weights['dosage_minimization']

            return deviation_penalty + dosage_penalty

        # Constraints
        # Individual dosage bounds
        bounds = Bounds(0, self.max_individual_dosage)
        ...
        try:
            ...
            if result.success:
               ...
               # Extract dosages
                ...
                # FIXED: Calculate deviations against ORIGINAL TARGETS (desired final concentrations)
                deviations_percent = {}
                for nutrient, target_final in targets.items():
                    achieved_final = achieved_concentrations.get(nutrient, 0.0)
                    if target_final > 0:
                        deviation = (
                            (achieved_final - target_final) / target_final) * 100.0
                    else:
                    ...
                    water_contrib = water.get(nutrient, 0.0)
                    print(
                        f"[LP-SciPy] {nutrient}: Target={target_final:.2f}, Achieved={achieved_final:.2f} (Water={water_contrib:.2f}), Dev={deviation:.2f}%")
    ...
    def _calculate_achieved_concentrations(self, dosages: Dict[str, float], 
        ...
        """
        Calculate final achieved concentrations including water baseline
        FIXED: Correctly adds water contributions to fertilizer contributions
        """
        achieved = {}
        
        # Start with water baseline
        for nutrient, water_value in water.items():
            achieved[nutrient] = water_value
        
        # Add fertilizer contributions
        for fert in fertilizers:
            dosage_g_l = dosages.get(fert.name, 0.0)
            if dosage_g_l > 0:
                # Add cation contributions
                for nutrient, content_percent in fert.composition.cations.items():
                    if content_percent > 0:
                        # FIXED: Correct contribution calculation
                        # g/L * (content% * purity% / 10000) * 1000 mg/g = g/L * content% * purity% * 0.1
                        contribution_mg_l = dosage_g_l * content_percent * fert.chemistry.purity * 0.1
                        achieved[nutrient] = achieved.get(nutrient, 0.0) + contribution_mg_l
                
                # Add anion contributions
                for nutrient, content_percent in fert.composition.anions.items():
                    if content_percent > 0:
                        # FIXED: Correct contribution calculation
                        contribution_mg_l = dosage_g_l * content_percent * fert.chemistry.purity * 0.1
                        achieved[nutrient] = achieved.get(nutrient, 0.0) + contribution_mg_l
    ...
    def _calculate_ionic_balance_error(self, concentrations: Dict[str, float]) -> float:
        ...
        cation_elements = ['Ca', 'K', 'Mg',
                           'Na', 'NH4', 'Fe', 'Mn', 'Zn', 'Cu']
        anion_elements = ['NO3', 'H2PO4', 'SO4', 'Cl', 'HCO3']

        try:
            cation_meq = sum(
                self.nutrient_calc.convert_mg_to_meq_direct(
                    concentrations.get(elem, 0), elem)
            ...
            if cation_meq > 0 or anion_meq > 0:
                balance_error = abs(cation_meq - anion_meq) / \
                    max(cation_meq, anion_meq, 1.0) * 100
            else:
                balance_error = 0.0
